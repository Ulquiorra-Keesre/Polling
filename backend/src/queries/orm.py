...
class UserRepository(DatabaseManager):
    ...
    async def update_user_role(
        self, student_id: str, new_role: UserRole
    ) -> Optional[User]:
...
class PollRepository(DatabaseManager):
    ...
    async def create_poll_with_options(
        ...
        try:
            poll = Poll(
                title=title, description=description, end_date=end_date, total_votes=0
            )
            self.session.add(poll)
            await self.session.flush()

            for option_text in options:
                option = Option(poll_id=poll.id, text=option_text, votes=0)
    ...
    async def update_poll_votes(self, poll_id: int) -> None:
        ...
        result = await self.session.execute(
            select(func.sum(Option.votes)).where(Option.poll_id == poll_id)
        )
        total_votes = result.scalar() or 0

        await self.session.execute(
            update(Poll).where(Poll.id == poll_id).values(total_votes=total_votes)
        )
        await self.session.commit()
...
class VoteRepository(DatabaseManager):
    ...
    async def get_user_votes_for_poll(
        self, poll_id: int, student_id: str
    ) -> List[Vote]:
    ...
    async def create_vote(
        self, poll_id: int, option_id: int, student_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            vote = await self.create(
                Vote, poll_id=poll_id, option_id=option_id, student_id=student_id
            )
            ...
            return {"vote": vote, "option": option, "poll_id": poll_id}
        except Exception:
            await self.session.rollback()
            raise
